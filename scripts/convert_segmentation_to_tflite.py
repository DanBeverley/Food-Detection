# scripts/convert_segmentation_to_tflite.py
...
def convert_h5_to_tflite(h5_model_path: str, tflite_output_path: str):
    """
    Converts a Keras model saved in .h5 format to TensorFlow Lite (.tflite) format.
    ...
    try:
        ...
        # Load the model. If there were custom layers/losses, they'd need custom_objects
        model = tf.keras.models.load_model(h5_model_path, compile=False) # No need to compile for conversion
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Keras .h5 model to TFLite format.")
    parser.add_argument(
        "--h5_model",
        type=str,
        required=True,
        help="Path to the input Keras model (.h5 file)."
    )
    parser.add_argument(
        "--tflite_output",
        type=str,
